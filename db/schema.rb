...
ActiveRecord::Schema[8.1].define(version: 20_260_502_210_002) do
  create_schema 'tiger'
  ...
  create_table 'tiger.addr', primary_key: 'gid', id: :serial, force: :cascade do |t|
    ...
    t.index %w[tlid statefp], name: 'idx_tiger_addr_tlid_statefp'
  ...
  create_table 'tiger.addrfeat', primary_key: 'gid', id: :serial, force: :cascade do |t|
    ...
    t.check_constraint "public.geometrytype(the_geom) = 'LINESTRING'::text OR the_geom IS NULL",
  ...
  create_table 'tiger.bg', primary_key: 'bg_id', id: { type: :string, limit: 12 }, comment: 'block groups',
                           force: :cascade do |t|
    t.float 'aland'
    t.float 'awater'
    ...
    t.check_constraint "public.geometrytype(the_geom) = 'MULTIPOLYGON'::text OR the_geom IS NULL",
  ...
  create_table 'tiger.county_lookup', primary_key: %w[st_code co_code], force: :cascade do |t|
  ...
  create_table 'tiger.countysub_lookup', primary_key: %w[st_code co_code cs_code], force: :cascade do |t|
    t.integer 'co_code', null: false
  ...
  create_table 'tiger.edges', primary_key: 'gid', id: :serial, force: :cascade do |t|
    ...
    t.check_constraint "public.geometrytype(the_geom) = 'MULTILINESTRING'::text OR the_geom IS NULL",
                       name: 'enforce_geotype_the_geom'
    t.check_constraint 'public.st_ndims(the_geom) = 2', name: 'enforce_dims_the_geom'
    t.check_constraint 'public.st_srid(the_geom) = 4269', name: 'enforce_srid_the_geom'
  end
  ...
  create_table 'tiger.featnames', primary_key: 'gid', id: :serial, force: :cascade do |t|
    ...
    t.index %w[tlid statefp], name: 'idx_tiger_featnames_tlid_statefp'
  end
  ...
  create_table 'tiger.loader_lookuptables', primary_key: 'lookup_name',
                                            id: { type: :text, comment: 'This is the table name to inherit from and suffix of resulting output table -- how the table will be named --  edges here would mean -- ma_edges , pa_edges etc. except in the case of national tables. national level tables have no prefix' }, force: :cascade do |t|
    t.text 'columns_exclude',
           comment: 'List of columns to exclude as an array. This is excluded from both input table and output table and rest of columns remaining are assumed to be in same order in both tables. gid, geoid,cpi,suffix1ce are excluded if no columns are specified.', array: true
  ...
  create_table 'tiger.loader_variables', primary_key: 'tiger_year', id: { type: :string, limit: 4 },
                                         force: :cascade do |t|
    t.text 'data_schema'
    t.text 'staging_fold'
  ...
  create_table 'tiger.place_lookup', primary_key: %w[st_code pl_code], force: :cascade do |t|
    t.string 'name', limit: 90
  ...
  create_table 'tiger.secondary_unit_lookup', primary_key: 'name', id: { type: :string, limit: 20 },
                                              force: :cascade do |t|
  ...
  create_table 'tiger.zcta5', primary_key: %w[zcta5ce statefp], force: :cascade do |t|
    t.float 'aland'
    t.float 'awater'
    t.string 'classfp', limit: 2
    t.string 'funcstat', limit: 1
  ...
  create_table 'tiger.zip_state', primary_key: %w[zip stusps], force: :cascade do |t|
    t.string 'statefp', limit: 2
    t.string 'stusps', limit: 2, null: false
    t.string 'zip', limit: 5, null: false
  end

  create_table 'tiger.zip_state_loc', primary_key: %w[zip stusps place], force: :cascade do |t|
